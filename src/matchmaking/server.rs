use crate::matchmaking::Protocol;
use std::{
    error,
...
pub trait ProtocolServer<P>
where
    P: Protocol,
{
    // Error type for this protocol server.
    type Error: error::Error + Send + Sync + 'static;

    // Start listening on the given `bind_to` address. For every new connection, a request must be
    // sent into the `new_connection_send` which will be routed to a backend server. The backend
    // server will then decide how to reply to that connection request.
    //
    ...
    // Insert the given connection and its state into the server. If successful, the server should
    // continue communicating through this socket stream as if nothing happened.
    ...
    // should still process existing connections as normal. And then for each existing connection,
    // it must:
    //
    // - Pause processing and flush the output buffer for that connection. The socket stream must
    //   not be closed.
    // - Move all resources into a `ServerConnectionState` struct.
    // - Send that struct through the channel and cleanup any remaining resources associated with
    //   that connection.
